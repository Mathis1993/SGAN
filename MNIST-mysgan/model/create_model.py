...
from tensorflow.keras.layers import Input, Dense, Reshape, Flatten, Conv2D, Conv2DTranspose, LeakyReLU, Dropout, Activation, Lambda
import numpy as np
...
#     # supervised output
#     c_out_layer = Dense(10, activation='softmax')(fe)
...
def define_discriminator(lr=0.0002, in_shape=(64,64,1), n_classes=10):
	# image input
	in_image = Input(shape=in_shape)
	# downsample
	fe = Conv2D(128, (3,3), strides=(2,2), padding='same')(in_image)
	...
	# define and compile supervised discriminator model
	c_model = Model(in_image, c_out_layer)
	c_model.compile(loss='sparse_categorical_crossentropy', optimizer=Adam(lr=lr, beta_1=0.5), metrics=['accuracy'])
	# unsupervised output
...
def define_baseline(lr, in_shape=(64,64,1)):
    ...
    # dropout
    fe = Dropout(0.4)(fe)
    b_out_layer = Dense(10, activation='softmax')(fe)
    # define and compile supervised discriminator model
    b_model = Model(in_image, b_out_layer)
    b_model.compile(loss='sparse_categorical_crossentropy', optimizer=Adam(lr=0.0002, beta_1=0.5), metrics=['accuracy'])
